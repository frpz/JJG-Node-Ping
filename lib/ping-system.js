...
exports.ping = function ping(address, callback) {
    ...
    process.stdout.on('data', function (data) {
        ...
        if (!array[time_param] || array.indexOf('Unreachable') > -1 || array.indexOf('exceeded') > -1) {
            status = false;
            latency = 0;
        }
        ...
        if (typeof status === 'boolean') {
            //callback && callback(latency, status);
			   res = {latency: latency, status: status};
        }
		  //console.log("ping: ", address, res);
    });
    ...
    process.stderr.on('data', function (data) {
        //callback && callback(0, false);
		  res = {latency: 0, status: false};
    });
    process.on('exit', function (data) {
        //callback && callback(0, false);
        callback && callback(res.latency, res.status);
    });
}
